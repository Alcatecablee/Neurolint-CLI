...
/**
 * TransformationLogger - Logs layer transformations for cross-session learning
 */
class TransformationLogger {
  constructor() {
    this.logPath = path.join(process.cwd(), '.neurolint', 'transformation-log.json');
    this.maxEntriesPerFile = 100;
    this.maxAgeDays = 30;
    this.maxLogSizeBytes = 5 * 1024 * 1024; // 5MB
  }

  async logTransformation(filePath, layerId, beforeCode, afterCode, changes) {
    try {
      // Don't log if no changes
      if (!changes || changes === 0 || beforeCode === afterCode) return;

      const entry = {
        timestamp: Date.now(),
        filePath: path.relative(process.cwd(), filePath),
        layerId,
        beforeCode: this._truncateCode(beforeCode),
        afterCode: this._truncateCode(afterCode),
        changes
      };
      ...
      // Cleanup old entries
      await this._cleanupLog(log);
      
      await this._saveLog(log);
    } catch (error) {
      // Silently fail - logging shouldn't break the main process
      if (process.env.NEUROLINT_DEBUG === 'true') {
        process.stderr.write(`[DEBUG] Failed to log transformation: ${error.message}\n`);
      }
    }
  }
  ...
  async _cleanupLog(log) {
    const now = Date.now();
    const maxAge = this.maxAgeDays * 24 * 60 * 60 * 1000;
    
    // Remove old entries
    log.entries = log.entries.filter(e => now - e.timestamp < maxAge);
    
    // Keep only last N entries per file
    const fileEntries = {};
    for (const entry of log.entries) {
      if (!fileEntries[entry.filePath]) fileEntries[entry.filePath] = [];
      fileEntries[entry.filePath].push(entry);
    }
    
    // Trim each file's entries
    const trimmedEntries = [];
    for (const [file, entries] of Object.entries(fileEntries)) {
      const sorted = entries.sort((a, b) => b.timestamp - a.timestamp);
      trimmedEntries.push(...sorted.slice(0, this.maxEntriesPerFile));
    }
    
    log.entries = trimmedEntries.sort((a, b) => b.timestamp - a.timestamp);
    
    // Check size limit
    const logSize = JSON.stringify(log).length;
    if (logSize > this.maxLogSizeBytes) {
      // Keep only most recent half
      log.entries = log.entries.slice(0, Math.floor(log.entries.length / 2));
    }
  }

  _truncateCode(code) {
    // Keep reasonable size for logging
  ...
  for (const layerNum of filteredLayers) {
    ...
    try {
      ...
      if (result) {
        ...
        // Log transformation for cross-session learning (Issue #4)
        if (result.success && (result.changeCount > 0 || result.changes?.length > 0) && !dryRun) {
          await transformationLogger.logTransformation(
            filePath,
            layerNum,
            previousCode,
            result.code,
            result.changeCount || result.changes?.length || 0
          );
        }
      }
