...
export function DocsLayerSecurity() {
  return (
    ...
    >
      ...
      <section className="mb-12">
        <h2 className="text-2xl font-bold text-white mb-4">Integration with Layer 7 (Adaptive Learning)</h2>
        
        <p className="text-gray-300 mb-4">
          Layer 8 security findings automatically feed into Layer 7's adaptive learning system. 
          When security vulnerabilities are detected, Layer 7 learns remediation patterns and 
          applies them automatically in future scans.
        </p>

        <Callout type="success" title="Automatic Security Learning">
          Security patterns are learned with 95% confidence (higher than regular patterns at 90%). 
          Once learned, similar vulnerabilities are automatically fixed without requiring Layer 8 
          to run again.
        </Callout>
          ...
          <div className="p-4 bg-zinc-900/50 border border-black rounded-lg">
            ...
            <ol className="space-y-2 text-sm text-gray-400">
              <li className="flex gap-2">
                ...
                <span>Layer 8 detects security findings (eval(), innerHTML, hardcoded credentials, etc.)</span>
              </li>
              <li className="flex gap-2">
                <span className="text-gray-500">2.</span>
                <span>Security findings are passed to Layer 7 with metadata and remediation hints</span>
              </li>
              <li className="flex gap-2">
                <span className="text-gray-500">3.</span>
                <span>Layer 7 extracts reusable patterns and stores them with securityRelated: true flag</span>
              </li>
              <li className="flex gap-2">
                <span className="text-gray-500">4.</span>
                <span>Future runs automatically apply learned security fixes (even without Layer 8)</span>
              </li>
            </ol>
          </div>

          <div className="p-4 bg-zinc-900/50 border border-black rounded-lg">
            <h3 className="font-medium text-white mb-2">Learned Security Patterns</h3>
            <ul className="space-y-1 text-sm text-gray-400">
              <li className="flex gap-2">
                <span className="text-gray-500">•</span>
                <span>eval() usage → Removed with security comment</span>
              </li>
              <li className="flex gap-2">
                <span className="text-gray-500">•</span>
                <span>innerHTML assignments → Replaced with textContent</span>
              </li>
              <li className="flex gap-2">
                <span className="text-gray-500">•</span>
                <span>dangerouslySetInnerHTML → Flagged for removal</span>
              </li>
              <li className="flex gap-2">
                <span className="text-gray-500">•</span>
                <span>Hardcoded credentials → Moved to environment variables</span>
              </li>
              <li className="flex gap-2">
                <span className="text-gray-500">•</span>
                <span>Command injection (exec) → Replaced with spawn</span>
              </li>
              <li className="flex gap-2">
                <span className="text-gray-500">•</span>
                <span>SQL injection patterns → Parameterized query hints</span>
              </li>
            </ul>
          </div>

          <div>
            <p className="text-sm text-gray-400 mb-2">Run Layer 8 with Layer 7 to enable learning:</p>
            <CommandBlock command="neurolint fix ./src --layers=8,7" />
            <p className="text-gray-400 text-xs mt-2">
              Layer 7 will automatically learn from security findings and apply them in future runs
            </p>
          </div>

          <div>
            <p className="text-sm text-gray-400 mb-2">View learned security rules:</p>
            <CommandBlock command="neurolint rules --list | grep securityRelated" />
          </div>
        </div>
      </section>
