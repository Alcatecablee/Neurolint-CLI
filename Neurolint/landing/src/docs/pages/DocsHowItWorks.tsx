...
Layer 7 (Adaptive)   → Applies & learns rules (cross-session learning enabled)
Layer 8 (Security)   → Scans for threats (read-only by default)`}
        ...
        <Callout type="info" title="Layer 7 Cross-Session Learning">
          Layer 7 now learns from individual layer runs! Each layer automatically logs 
          transformations to <code className="text-white">.neurolint/transformation-log.json</code>. 
          When Layer 7 runs, it extracts patterns from this history and applies them 
          automatically. No need to run all layers together for learning to work.
        </Callout>
      </section>
