...
export function DocsArchitecture() {
  const layers = [
    ...
    },
    {
      num: 7,
      name: "Adaptive",
      description: "Learns and applies patterns from prior fixes (cross-session enabled)",
      fixes: [
        "Cross-session learning - logs transformations from individual layer runs",
        ...
        "Logs transformations to .neurolint/transformation-log.json with automatic rotation",
        "Applies patterns only when confidence exceeds 70% threshold",
        "Security patterns learned from Layer 8 with 95% confidence",
        "Manages rules via --list, --edit, --delete, --reset, --export, --import commands",
        "Debug logging available via NEUROLINT_DEBUG environment variable",
      ],
      engine: "RuleStore + TransformationLogger + CrossSessionLearningManager with AST diff analysis",
